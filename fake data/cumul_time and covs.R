...
#let's assume that we have some NA's on covariates
n=10
xsel=sample(xdim,size=n)
ysel=sample(ydim,size=n)
for (i in 1:n){
  ind=sample(1:ncov,size=1)
  xmat[xsel[i],ysel[i],ind]=NA  
}
...
for (i in 1:(nsim-1)){
  ...
  #if no NA in covariates and time.int<=8 min 
  cond.not.na=sum(is.na(res1[,'cum.time']))==0
  cond.time.int=coord[i,'time.int']>=6 & coord[i,'time.int']<=8
